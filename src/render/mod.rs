...
struct UniformData { mvp_matrix: [[f32; 4]; 4] }
...
impl<'a> Renderer<'a> {
    pub fn new(window: Arc<Window>) -> Self {
        ...
        // Notice the macro now correctly targets the file locally in this folder
        ...
        // Pull geometry from your brand new modules!
        let grid_verts = crate::floor::create_vertices();
        let cube_verts = crate::player::create_vertices();
        let star_verts = crate::sky::create_vertices();

        let grid_vertex_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: None, contents: bytemuck::cast_slice(&grid_verts), usage: wgpu::BufferUsages::VERTEX });
        ...
        let grid_uniform_buffer = device.create_buffer(&wgpu::BufferDescriptor { label: None, size: matrix_size, usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false });
        let grid_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor { layout: &uniform_bind_group_layout, entries: &[wgpu::BindGroupEntry { binding: 0, resource: grid_uniform_buffer.as_entire_binding() }], label: None });
        ...
        Self {
            ...
            grid_vertex_buffer, grid_vertex_count: grid_verts.len() as u32,
            cube_vertex_buffer, cube_vertex_count: cube_verts.len() as u32,
            star_vertex_buffer, star_vertex_count: star_verts.len() as u32,
            grid_uniform_buffer, grid_bind_group, cube_uniform_buffer, cube_bind_group, projection_matrix,
        }
    }
    ...
    pub fn update_matrices(&self, player_pos: Vec3, camera_yaw: f32, camera_pitch: f32, camera_dist: f32) {
        ...
        self.queue.write_buffer(&self.grid_uniform_buffer, 0, bytemuck::cast_slice(&[UniformData { mvp_matrix: (view_proj * Mat4::IDENTITY).to_cols_array_2d() }]));
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: None,
                color_attachments: &[Some(wgpu::RenderPassColorAttachment { view: &view, resolve_target: None, ops: wgpu::Operations { load: wgpu::LoadOp::Clear(crate::core::colors::SKY_BLACK), store: wgpu::StoreOp::Store } })],
