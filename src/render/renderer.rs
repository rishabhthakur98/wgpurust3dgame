...
pub struct Renderer<'a> {
    ...
    // NEW CLEANUP: We wiped all the hardcoded buildings! Now we just have the ground model.
    ...
    cube_count: u32, 
    player_side_tex: Texture, 
    player_top_tex: Texture,
...
impl<'a> Renderer<'a> {
    pub fn new(window: Arc<Window>) -> Self {
        ...
        let skybox_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor { 
            label: Some("Skybox"), layout: Some(&skybox_pipeline_layout), vertex: wgpu::VertexState { module: &skybox_shader, entry_point: "vs_main", buffers: &[SkyboxVertex::desc()], compilation_options: Default::default() }, fragment: Some(wgpu::FragmentState { module: &skybox_shader, entry_point: "fs_main", targets: &[Some(config.format.into())], compilation_options: Default::default() }), primitive: wgpu::PrimitiveState { topology: wgpu::PrimitiveTopology::TriangleList, cull_mode: None, ..Default::default() }, depth_stencil: Some(wgpu::DepthStencilState { format: wgpu::TextureFormat::Depth32Float, depth_write_enabled: false, depth_compare: wgpu::CompareFunction::LessEqual, stencil: Default::default(), bias: Default::default() }), multisample: wgpu::MultisampleState::default(), multiview: None 
        });

        // LOAD FROM THE NEW GLTF SYSTEM
        let world_state = crate::world::WorldState::new();
        let g_verts = world_state.ground_vertices;
        let c_verts = crate::player::create_vertices();
        let sky_verts = crate::sky::create_skybox_cube();

        let ground_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: None, contents: bytemuck::cast_slice(&g_verts), usage: wgpu::BufferUsages::VERTEX });
        ...
        // Generate the fallback white texture so untextured models can render properly
        let ground_tex = Texture::create_white_texture(&device, &queue, "white_fallback", &texture_layout);
        ...
        Self {
            ...
            ground_buffer, ground_count: g_verts.len() as u32, ground_tex,
    ...
    pub fn update_matrices(&self, player_pos: Vec3, player_yaw: f32, camera: &crate::camera::Camera, is_day: bool, is_flashlight_on: bool, is_freeform: bool) {
        ...
        let sun_dir_vec = Vec3::new(el.cos() * az.sin(), el.sin(), el.cos() * az.cos()).normalize();
        ...
        let cam_pos = if is_freeform { camera.pos } else { player_pos + Vec3::new(camera.yaw.sin() * camera.pitch.cos() * camera.distance, camera.pitch.sin() * camera.distance, camera.yaw.cos() * camera.pitch.cos() * camera.distance) };

        let light_target = if is_freeform { camera.pos } else { player_pos };
        let light_pos = light_target + (sun_dir_vec * 600.0);
        ...
        let mut view_matrix = if is_freeform { Mat4::look_at_rh(camera.pos, camera.pos + forward_dir, Vec3::Y) } else { Mat4::look_at_rh(cam_pos, player_pos, Vec3::Y) };
        view_matrix.w_axis = glam::Vec4::new(0.0, 0.0, 0.0, 1.0);
        
        let z_c = crate::sky::config::SKY_ZENITH_DAY; let h_c = crate::sky::config::SKY_HORIZON_DAY; let n_c = crate::sky::config::SKY_NIGHT;

        let world_model = Mat4::IDENTITY;
        self.queue.write_buffer(&self.world_uniform_buffer, 0, bytemuck::cast_slice(&[UniformData { 
            mvp_matrix: (view_proj * world_model).to_cols_array_2d(), model_matrix: world_model.to_cols_array_2d(), 
            light_mvp_matrix, sun_dir, sun_color, ambient_color, point_lights,
            flashlight_pos: [cam_pos.x, cam_pos.y, cam_pos.z, 1.0], flashlight_dir: [forward_dir.x, forward_dir.y, forward_dir.z, 0.0], flashlight_color,
            sky_mvp_matrix: (self.projection_matrix * view_matrix).to_cols_array_2d(),
            sky_zenith: [z_c[0], z_c[1], z_c[2], 1.0], sky_horizon: [h_c[0], h_c[1], h_c[2], 1.0], sky_night: [n_c[0], n_c[1], n_c[2], 1.0]
        }]));
        ...
        self.queue.write_buffer(&self.cube_uniform_buffer, 0, bytemuck::cast_slice(&[UniformData { 
            mvp_matrix: (view_proj * cube_model).to_cols_array_2d(), model_matrix: cube_model.to_cols_array_2d(), 
            light_mvp_matrix, sun_dir, sun_color, ambient_color, point_lights,
            flashlight_pos: [cam_pos.x, cam_pos.y, cam_pos.z, 1.0], flashlight_dir: [forward_dir.x, forward_dir.y, forward_dir.z, 0.0], flashlight_color,
            sky_mvp_matrix: (self.projection_matrix * view_matrix).to_cols_array_2d(),
            sky_zenith: [z_c[0], z_c[1], z_c[2], 1.0], sky_horizon: [h_c[0], h_c[1], h_c[2], 1.0], sky_night: [n_c[0], n_c[1], n_c[2], 1.0]
    ...
    pub fn render(&mut self, _is_day: bool, is_freeform: bool) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            shadow_pass.set_vertex_buffer(0, self.ground_buffer.slice(..)); 
            shadow_pass.draw(0..self.ground_count, 0..1);
...

        {
            ...
            pass.set_bind_group(0, &self.world_bind_group, &[]);
            pass.set_bind_group(1, &self.ground_tex.bind_group, &[]); 
